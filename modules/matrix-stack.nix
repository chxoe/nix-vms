{nginx, element, matrix}: {pkgs, config, ...}: {

	networking.firewall.allowedTCPPorts = [ 80 443 ];

	security.acme = {
		acceptTerms = true;
		defaults.email = nginx.acme-email;
	};
	...
	services = {
			...
			virtualHosts = {
				"${matrix.domain}" = {
					...
					locations."/".extraConfig = ''
						return 301 https://${element.domain}/;
					'';
		...
		matrix-synapse = {
			enable = true;
			settings.server_name = "${matrix.domain}";
			settings.public_baseurl = "https://${matrix.domain}";
			settings.listeners = [
				{
					port = 8008;
			...
			settings.registration_shared_secret = "${matrix.registration-secret}";
	...
	systemd.services = {
		...
		lk-jwt-service.environment.LIVEKIT_FULL_ACCESS_HOMESERVERS = "${matrix.domain}";
	};
}
